...
def Init():
    ...
    global HANGMAN_PICS,Win,missedLetters,correctLetters,Z
   ...
   / \  |
    ...
    Z = 1
def Initiate(event):
    global title1,title2,title6,title7,Var,Var1,String
    ...
    while Type[0]==" ":
    # This loop is to remove any spaces added to the input as the only input allowed is one character 
        Type=Type[1:]
    if len(Type)>1:
    # This if statement is to make sure that more than one letter is not allowed in the input
        title2.configure(text="Enter one letter only")
        Var.set("")
        return
    global word,correctLetters,missedLetters,l
